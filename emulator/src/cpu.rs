...
impl CPU {
    ...
    pub fn execute_instruction(&mut self) -> InstructionResult {
        ...
        if self.memory_bus.hdma().active && self.memory_bus.hdma().general_purpose {
            self.memory_bus.tick(4);
            return InstructionResult {
                cycles: self.memory_bus.total_cycles(),
            };
        }

        let interrupt_pending = (self.memory_bus.if_flag() & self.memory_bus.ie_flag() & 0x1F) != 0;
        if interrupt_pending && self.memory_bus.halted() {
            self.memory_bus.set_halted(false);
        }
        ...
        if opcode == 0xCB {
            ...
            match cb_instruction.mnemonic {
                ...
                _ => console_error!(
                    "Unknown CB-prefixed opcode: {:02X} {:?}",
                    cb_opcode,
                    cb_instruction.mnemonic
                ),
            };
    ...
    fn interrupt(&mut self) {
        ...
        self.memory_bus
            .set_if_flag(self.memory_bus.if_flag() & (!interrupt_num));

        self.memory_bus.tick(8);

        self.stack16(self.registers.get(Register::PC));
        self.registers
            .set(Register::PC, (0x0040 + interrupt_bit * 8) as u16);
            ...
            None => {
                match operand.name {
                    OperandName::A8 => {
                        ...
                        return (
                            self.memory_bus.read(self.sanitize_address(address, high)) as u16,
                            1,
                        );
                    }
                    OperandName::E8 => return (self.read_e8(), 1),
                    _ => {
                        if !operand.immediate {
        ...
        match operand.register {
            ...
            None => {
                ...
                if register_bytes == 1 {
                    ...
                    self.memory_bus
                        .write(sanitized_address, (value & 0xFF) as u8);
                    self.memory_bus
                        .write(sanitized_address + 1, (value >> 8) as u8);
                }
            }
        }
    }
    ...
    fn noop(&mut self, instruction: &Instruction) {
        let pc = self.registers.get(Register::PC);
        self.registers
            .set(Register::PC, pc + instruction.bytes as u16);
    }
    ...
    fn rst(&mut self, instruction: &Instruction) {
        ...
        self.registers
            .set(Register::PC, (self.memory_bus.read(pc) - 0xC7) as u16);
    }
    ...
    fn jr(&mut self, instruction: &Instruction) {
        ...
        self.registers.set(
            Register::PC,
            pc.wrapping_add(instruction.bytes as u16)
                .wrapping_add(offset),
        );
        self.memory_bus.tick(4);
    }
    ...
    fn ld(&mut self, instruction: &Instruction) {
        ...
        if instruction.total_operands == 3 {
            ...
            match instruction.operands[0].register {
                Some(Register::SP) => {
                    if !matches!(instruction.operands[1].register, None)
        ...
        for i in 0..(instruction.total_operands as usize) {
            if instruction.operands[i].increment {
                ...
                self.registers
                    .set(register, self.registers.get(register).wrapping_add(1));
            } else if instruction.operands[i].decrement {
                let register = instruction.operands[i].register.unwrap();
                self.registers
                    .set(register, self.registers.get(register).wrapping_sub(1));
            }
        }
    ...
    fn inc(&mut self, instruction: &Instruction) {
        ...
        if instruction.operands[0].immediate {
            match instruction.operands[0].register {
                Some(Register::AF) | Some(Register::BC) | Some(Register::DE)
                | Some(Register::HL) | Some(Register::SP) => self.memory_bus.tick(4),
                _ => {}
            };
        }
    ...
    fn or(&mut self, instruction: &Instruction) {
        ...
        self.registers
            .set_zero(self.registers.get(Register::A) == 0);
        self.registers.set_subtract(false);
    ...
    fn add(&mut self, instruction: &Instruction) {
        ...
        if instruction.operands[0].immediate {
            match instruction.operands[0].register {
                Some(Register::AF) | Some(Register::BC) | Some(Register::DE)
                | Some(Register::HL) => self.memory_bus.tick(4),
    ...
    fn rrc(&mut self, instruction: &Instruction) {
        ...
        self.registers
            .set_zero(new_value == 0 && !matches!(instruction.mnemonic, Mnemonic::RRCA));
        self.registers.set_subtract(false);
        self.registers.set_half_carry(false);
        self.registers.set_carry(value & 0x01 != 0);
    ...
    fn rr(&mut self, instruction: &Instruction) {
        ...
        self.registers
            .set_zero(new_value == 0 && !matches!(instruction.mnemonic, Mnemonic::RRA));
        self.registers.set_subtract(false);
        self.registers.set_half_carry(false);
        self.registers.set_carry(value & 0x01 != 0);
    ...
    fn rl(&mut self, instruction: &Instruction) {
        ...
        self.registers
            .set_zero(new_value == 0 && !matches!(instruction.mnemonic, Mnemonic::RLA));
        self.registers.set_subtract(false);
        self.registers.set_half_carry(false);
        self.registers.set_carry(value & 0x80 != 0);
    ...
    fn rlc(&mut self, instruction: &Instruction) {
        ...
        self.registers
            .set_zero(new_value == 0 && !matches!(instruction.mnemonic, Mnemonic::RLCA));
        self.registers.set_subtract(false);
        self.registers.set_half_carry(false);
        self.registers.set_carry(value & 0x80 != 0);
    ...
    fn halt(&mut self, instruction: &Instruction) {
        self.memory_bus.set_halted(true);
        let pc = self.registers.get(Register::PC);
        self.registers
            .set(Register::PC, pc + instruction.bytes as u16);
    }
    ...
    fn stop(&mut self, instruction: &Instruction) {
        ...
        if matches!(
            self.memory_bus.model_type(),
            memory_bus::types::ModelType::Color
        ) && self.memory_bus.key1() & 0x01 != 0
        {
...
mod tests {
    ...
    pub fn test_push_af() {
        ...
        assert_eq!(sp, INITIAL_SP - 2);
        assert_eq!(
            cpu.memory_bus.read(sp),
            cpu.registers.get(Register::F) as u8
        );
        assert_eq!(
            cpu.memory_bus.read(sp + 1),
            cpu.registers.get(Register::A) as u8
        );
        assert_eq!(cpu.registers.get(Register::PC), INITIAL_PC + 1);
    ...
    pub fn test_push_r16() {
        ...
        assert_eq!(
            cpu.memory_bus.read(sp),
            cpu.registers.get(Register::C) as u8
        );
        assert_eq!(
            cpu.memory_bus.read(sp + 1),
            cpu.registers.get(Register::B) as u8
        );
        assert_eq!(cpu.registers.get(Register::PC), INITIAL_PC + 1);
    ...
    pub fn test_halt_sets_halted_flag() {
        ...
        assert_eq!(cpu.memory_bus.halted(), false);

        let result = cpu.execute_instruction();

        assert_eq!(cpu.memory_bus.halted(), true);
        assert_eq!(cpu.registers.get(Register::PC), INITIAL_PC + 1);
        assert_eq!(result.cycles, 4);
    }
